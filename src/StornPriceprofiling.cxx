...
double Corana(const EArray<double> &x) {
...
double HyperEllipsoid(const EArray<double> &x) {
    double sum1 = 0;
    for (auto i = 0; i < x.size(); ++i) {
...
double IntTest(const EArray<double> &x) {
...
class StornTest{
protected:
    std::function<double(const EArray<double>&)> m_f;
    int m_NP;
...
public:
    int Ncalls = 0;
    StornTest(std::function<double(const EArray<double>&)> &f, int NP, double F, double CR, double VTR, std::vector<CEGO::Bound> bounds)
...
int main() {
    {
        // Integer problem #1
        std::function<double(const EArray<double> &)> f = IntTest;
    ...
    { 
        // Test problem 6
        std::function<double(const EArray<double> &)> f = Corana;
    ...
    {
        // Test problem 7
        std::function<double(const EArray<double> &)> f = Griewangk<double>;
    ...
    }
    {
        ...
        std::function<double(const EArray<double> &)> f = HyperEllipsoid;
        for (int D : {30, 100}){
